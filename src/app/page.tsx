...
export default function Home() {
  ...
  const [onlineUsers, setOnlineUsers] = useState(1); // Kendini her zaman 1 say
  ...
  // 🚀 SAYAÇLARI VERİTABANINDAN ÇEK VE CANLI DİNLE
  useEffect(() => {
    // 1. Toplam "Eller Havada" (Mesaj) sayısını al
    const fetchTotalCount = async () => {
      const { count, error } = await supabase
        .from('spots')
        .select('*', { count: 'exact', head: true }); // Sadece sayıyı çeker (Hızlıdır)
      
      if (!error && count !== null) {
        setTotalHands(count); // Sahte 13.842 yerine gerçek sayıyı koy
      }
    };
    fetchTotalCount();

    // 2. Biri yeni mesaj attığında sayacı anında (F5'siz) +1 artır
    ...
    // 3. Online Kullanıcı Sayısı (Presence)
    // Bu, siteyi o an açık tutan tarayıcıları sayar!
    const presenceChannel = supabase.channel('online-users');
    
    presenceChannel
      .on('presence', { event: 'sync' }, () => {
        const state = presenceChannel.presenceState();
        // Supabase'deki bağlı benzersiz cüzdan/sekme sayısını sayıyoruz
        const count = Object.keys(state).length;
        setOnlineUsers(count === 0 ? 1 : count); // Kimse yoksa bile sen varsın (1)
      })
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          await presenceChannel.track({ user: 'anonymous' }); // Sisteme "Ben girdim" de
        }
      });
  ...
  const handleCheckIn = async (e: React.FormEvent) => {
    ...
    if (error) {
      alert("Hata oluştu! Belki bu koltuk doludur, tekrar dene.");
      console.error(error);
    } else {
      setShowModal(false);
      setMessage("");
  ...
  return (
    <main className="h-screen w-screen flex flex-col font-sans overflow-hidden">
      ...
      <div className={`h-[35vh] w-full relative flex items-end justify-center overflow-hidden ${getSkyTheme()}`}>
        ...
        {timeMode === "night" && <div className="absolute inset-0 opacity-40 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-zinc-800 to-transparent" />}

        <div className="relative w-full max-w-6xl h-[85%] flex items-end justify-center px-4">
          <div className="hidden sm:flex flex-col justify-end w-24 md:w-32 h-full bg-zinc-950 border-4 border-zinc-800 rounded-t-lg z-20 shadow-2xl relative group">
          ...
          <div className="w-[90%] sm:w-[60%] h-full bg-zinc-900 border-t-8 border-x-8 border-zinc-950 rounded-t-[3rem] sm:rounded-t-[5rem] flex items-center justify-center z-10 relative shadow-[0_-10px_50px_rgba(0,0,0,0.5)] overflow-hidden mx-[-10px]">
            ...
            <div className="w-[85%] h-[75%] bg-black border-4 border-zinc-800 rounded-xl relative overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.8)] z-10 group cursor-pointer">
              <iframe 
                className="w-full h-full object-cover pointer-events-none group-hover:pointer-events-auto"
                src="https://www.youtube.com/embed/5qap5aO4i9A?autoplay=0&controls=1&mute=0&loop=1" 
      ...
      {/* ⚡ KONTROL ÇİZGİSİ (GERÇEK SAYAÇLAR BURADA) */}
      <div className="h-10 w-full bg-zinc-950 border-y-2 border-zinc-900 flex items-center justify-center px-4 z-30 shadow-[0_5px_20px_rgba(0,0,0,0.9)] relative">
        <div className="absolute inset-0 opacity-20 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iNDAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGxpbmUgeDE9IjEwIiB5MT0iMCIgeDI9IjEwIiB5Mj0iNDAiIHN0cm9rZT0iI2ZmZiIgc3Ryb2tlLXdpZHRoPSIyIi8+PC9zdmc+')]"/>
        ...
        <div className="flex items-center gap-6 z-10 bg-black/80 px-6 py-1 rounded-full border border-zinc-800">
          ...
          <div className="flex items-center gap-2 text-zinc-400 text-xs sm:text-sm font-medium border-l border-zinc-800 pl-6 tracking-widest transition-all">
      ...
      {/* 🚨 CHECK-IN POPUP MODALI */}
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="bg-zinc-950 border-2 border-zinc-800 w-full max-w-md rounded-2xl p-6 relative shadow-[0_0_50px_rgba(255,0,127,0.2)]">
